...
// POST /.netlify/functions/comments      -> agrega un comentario nuevo { name, course, school, rating, comment }
...
// En algunos sitios Netlify no inyecta automáticamente el contexto de Blobs
// en las funciones. Si existen BLOBS_SITE_ID y BLOBS_TOKEN como variables de
// entorno del sitio, se usan para configurar el store manualmente; si no,
// se recurre a la configuración automática estándar.
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'POST') {
    ...
    const school = (payload.school || '').toString().trim().slice(0, 120);
